...
- **A command bar that never moves the list.** It sits above the rows at a fixed height in *every* state — nothing selected, loading, empty, failed — and crossfades between the item count and the commands instead of appearing and shoving the rows down, so the row you just clicked is still under the pointer when you reach for the next one. It offers only what the selection can actually carry out: rename needs exactly one item, copy is files-only because the server refuses a folder copy, and download changes shape with the selection — one file is the browser's own download, anything else is a zip. On a narrow window the commands become icons and scroll sideways rather than wrapping onto a second line — wrapping would move the list, which is the thing this exists to prevent.
...
- **Sign in with Google or an email and password**, with email verification, Argon2id password hashing, durable rate limits and an Argon2 concurrency semaphore. A Google sign-in whose address already has a verified account links to that account rather than opening a second one, an account that has only ever signed in with Google can add a password through Forgot password, and Google shows an "unverified app" notice on the way through — the consent screen has not been through Google's review: three non-sensitive scopes, no logo, one portfolio deployment.
...
- **Zip downloads, built in the browser.** A folder — or any selection of more than one thing — comes down as a single zip that this tab assembles: it walks the folders through the API, fetches each file straight off the object store over its own short-lived link, and writes the archive entry by entry, empty folders included. There is no server-side archive endpoint and there is not going to be one — the bytes never pass through the app, which is the rule the whole thing is built on. On Chromium it streams into the file you picked as it is built, so the size is bounded by your disk; every other browser has to assemble it in memory, and past 1 GB the app says so and offers the files one at a time rather than killing the tab. One file on its own stays a plain download: wrapping it in a zip would cost the browser's own progress and resume and buy nothing.
