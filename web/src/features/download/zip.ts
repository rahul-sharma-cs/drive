 ...
 *  - a **stream** (`writeArchive`, in `archiveStream.ts`) — each file's bytes
 *    fetched from the store and fed to client-zip one at a time, so nothing is
 *    held in memory that is not on its way out;
 *  - a **sink** (`diskSink` / `blobSink`) — a real file on disk where the
 *    browser allows it, an in-memory Blob where it does not.
 *
 * The stream is the one piece that lives in its own file, and only because it
 * is the one piece that needs client-zip: keeping it out of here is what keeps
 * the library out of the bundle a person waits for before they see their files.
 *
